...
#ROOT.setTDRStyle()
#plot_vars = ["hEtaPlus" , "hEtaMinus" , "h0Barrel" , "h0EndcapPlus" , "h0EndcapMinus" , "gammaBarrel", "gammaEndcapPlus" , "gammaEndcapMinus" ] #"h_HF_Plus" , "h_HF_Minus" ,  "egamma_HF_Plus" , "egamma_HF_Minus"
#plot_vars = ["h_HF_Plus", "h_HF_Minus","egamma_HF_Plus" , "egamma_HF_Minus"]
plot_vars = ["gammaEndcapMinus"]
p_vars = ["x","y"]
#p_vars = ["y"]
...
for p_var in p_vars:
  for var in plot_vars:
    ...
    {"histo":h_w_f,   "name":"WJets Fall15",   "line":2,"color":ROOT.kYellow},\
    ]

    path = "/afs/hephy.at/user/e/easilar/www/METPhiCorr/plots/" 
    if not os.path.exists(path):
      os.makedirs(path)
    cb = ROOT.TCanvas("cb","cb",600,600,600,600)
    #h_dy.Draw()
    leg = ROOT.TLegend(0.65,0.6,0.87,0.87)
    leg.SetBorderSize(1)
    for histo in histo_list :
      print histo["name"]
      histo["histo"].SetLineColor(histo["color"])
      histo["histo"].SetLineWidth(histo["line"])
      histo["histo"].GetXaxis().SetRangeUser(0, 100)
      histo["histo"].GetYaxis().SetRangeUser(-1, 1)
    ...
    leg.Draw()
    cb.Draw()
    cb.SaveAs(path+var+"_P"+p_var+"_"+file_var+".pdf")
    cb.SaveAs(path+var+"_P"+p_var+"_"+file_var+".png")
